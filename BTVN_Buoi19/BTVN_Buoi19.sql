...
SELECT e.name AS EmployeeName, m.name AS Manager_Name, p.project_name
FROM employees e
JOIN employee_projects ep ON e.employee_id=ep.employee_id
JOIN projects p ON ep.project_id=p.project_id
LEFT JOIN employees m ON e.manager_id=m.employee_id;
-- 6) Liệt kê tên phòng ban và số lượng nhân viên tham gia dự án của từng phòng ban
SELECT d.department_name, COUNT( DISTINCT e.employee_id) AS soLuongNhanVien
FROM employees e
JOIN departments d ON e.department_id=d.department_id
JOIN employee_projects ep ON e.employee_id=ep.employee_id
GROUP BY d.department_id,d.department_name
-- 7) Tìm tên nhân viên có lương cao nhất tham gia trong mỗi dự án
SELECT p.project_name,e.name AS Employee_Name, MAX(e.salary) AS Max_Salary
FROM employees e
JOIN employee_projects ep ON e.employee_id=ep.employee_id
JOIN projects p ON ep.project_id=p.project_id
...
ORDER BY Max_Salary DESC 
 ...
 SELECT p.project_name, COUNT(e.employee_id) AS so_luong_nhan_vien
 FROM employees e
 JOIN employee_projects ep ON e.employee_id=ep.employee_id
 JOIN projects p ON ep.project_id=p.project_id
 ...
 SELECT p.project_name, d.department_name, e.name
 FROM employees e
 JOIN departments d ON e.department_id=d.department_id
 JOIN employee_projects ep ON e.employee_id=ep.employee_id
 JOIN projects p ON ep.project_id=p.project_id
 GROUP BY p.project_name, d.department_name, e.name
 --11) Tìm tên nhân viên và số lượng dự án mà họ tham gia nhiều nhất
 SELECT TOP 1 e.name, COUNT(p.project_id) AS Number_Of_Projects
 FROM employees e
  JOIN employee_projects ep ON e.employee_id=ep.employee_id
 JOIN projects p ON ep.project_id=p.project_id
 GROUP BY e.name
...
SELECT p.project_name, e.name, MIN(e.salary) AS Minimum_Salary
FROM employees e
JOIN employee_projects ep ON e.employee_id=ep.employee_id
JOIN projects p ON ep.project_id=p.project_id
...
SELECT e.name, d.department_name,MAX(e.salary) AS Max_Salary , MIN (e.salary) AS Min_Salary 
FROM employees e
JOIN departments d ON e.department_id=d.department_id
GROUP BY d.department_name, e.name
-- 16) Tính tổng lương và số lượng nhân viên cho từng dự án trong mỗi phòng ban
SELECT d.department_name, p.project_name, SUM(e.salary) AS TotalSalary, COUNT(e.employee_id) Number_Of_Employees
FROM employees e
JOIN departments d ON e.department_id=d.department_id
JOIN employee_projects ep ON e.employee_id=ep.employee_id
JOIN projects p ON ep.project_id=p.project_id
...
SELECT d.department_name, SUM(p.project_id) AS Total_Projects
FROM departments d
JOIN projects p ON d.department_id=p.department_id
GROUP BY d.department_name
...
WHERE EXISTS (
  ...
  WHERE subquery.department_name = d.department_name AND subquery.max_salary = e.salary
);
